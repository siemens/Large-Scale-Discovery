...
import (
	...
	"github.com/siemens/Large-Scale-Discovery/broker/config"
	"github.com/siemens/Large-Scale-Discovery/broker/core"
...
func main() {
	...
	// Initialize config
	errConf := config.Init("broker.conf")
	if errConf != nil {
	...
	gracy.Register(func() {
		...
		logger.Debugf("Broker terminated.")
	})
	...
	errInit := core.Init()
	if errInit != nil {
		logger.Errorf("Could not initialize broker: %s", errInit)
		return
	}
	...
	errServe := core.Run()
	if errServe != nil {
		logger.Errorf("Could not serve broker RPC: %s", errServe)
	}
}
...
func buildInfo() []string {
	return []string{
		...
		fmt.Sprintf("Broker API Version: %s", core.BrokerApiVersion.String()),
	}
}
