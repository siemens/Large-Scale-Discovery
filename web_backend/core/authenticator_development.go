...
func init() {
	if _build.DevMode {
		...
		authenticators = append(authenticators, NewAuthenticatorDevelopment(nil))
	}
}

type AuthenticatorDevelopment struct {
	domains  []string
	entryUrl string
}

// NewAuthenticatorDevelopment generates a new authenticator with the user domains it is responsible for. Everything
// else of the authenticator will be initialized later during core initialization, with the actual config values.
func NewAuthenticatorDevelopment(domains []string) *AuthenticatorDevelopment {
	return &AuthenticatorDevelopment{
		domains: domains,
	}
}

// Domains returns the user domains this authenticator got registered for
func (a *AuthenticatorDevelopment) Domains() []string {
	return a.domains
}
...
//    - /v1/auth/register
//    - /v1/auth/login
func (a *AuthenticatorDevelopment) Init(conf map[string]interface{}) error {
...
func (a *AuthenticatorDevelopment) EntryUrl() string {
	return a.entryUrl
}

// register creates a new user for credential authentication
func (a *AuthenticatorDevelopment) register() gin.HandlerFunc {
	...
	return func(context *gin.Context) {
		...
		// Log login attempt
		logger.Debugf("Development user '%s' trying to register.", req.Email)
...
func (a *AuthenticatorDevelopment) login() gin.HandlerFunc {
	...
	return func(context *gin.Context) {
		...
		// Log login attempt
		logger.Debugf("Development user '%s' trying to authenticate.", req.Email)
