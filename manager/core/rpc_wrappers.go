...
func RpcUpdateScopeTargets(
	...
	if rpcReply.Error != "" {
		return ReplyTargetsUpdate{}, fmt.Errorf("%s", rpcReply.Error)
	}
...
// RpcUpdateScopeTargetOt updates the input and input_size of an OT discovery scope's single
// t_discovery entry with data fed back by the scan agent after a scan.
func RpcUpdateScopeTargetOt(
	logger scanUtils.Logger,
	rpc *utils.Client,
	scopeId uint64,
	input string,
	...
	// Prepare RPC request
	rpcEndpoint := "Manager.UpdateScopeTargetOt"
	rpcReply := struct{}{}
	rpcArgs := ArgsScopeTargetUpdateOt{
		IdTScanScopes: scopeId,
		Input:         input,
		InputSize:     inputSize,
	}
	...
	// Send RPC request
	errRpc := rpc.Call(logger, rpcEndpoint, rpcArgs, &rpcReply)
	if errRpc != nil {
		return errRpc
	}
...
// RpcUpdateScopeStats updates scan agent stats and queue stats of a certain scope in the manager's db.
func RpcUpdateScopeStats(
	logger scanUtils.Logger,
	rpc *utils.Client,
	agentStatsByScope map[uint64][]database.T_scan_agent,
	scopeStatsByScope map[uint64]utils.JsonMap,
) error {

	// Prepare RPC request
	rpcEndpoint := "Manager.UpdateScopeStats"
	rpcReply := struct{}{} // Does not return values
	rpcArgs := ArgsUpdateScopeStats{
		ScanAgents: agentStatsByScope,
		ScopeStats: scopeStatsByScope,
	}
