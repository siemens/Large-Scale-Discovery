...
var viewDefinitions = map[string]string{
	...
	"nuclei": `
		...
		SELECT
			...
			t_nuclei.scan_started,
			t_nuclei.scan_finished,
			t_nuclei.scan_status,
			t_nuclei.scan_ip,
			t_nuclei.scan_hostname,
			...
			t_discovery_services.ad_os_version,
			t_discovery_services.ad_service_principal_name
		FROM t_nuclei_results
			LEFT JOIN t_discovery_services ON t_nuclei_results.id_t_discovery_service = t_discovery_services.id
			LEFT JOIN t_nuclei ON t_nuclei_results.id_t_nuclei = t_nuclei.id;
	`,
